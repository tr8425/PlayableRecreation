...
| 7 | **몰입 모드 2칸** (예전 "리얼리티 모드") — 상대를 고른 뒤 두 폰을 판 앞에 붙잡아 두고, 대화·사교 다툼·상대의 죽음·방문객까지 이야기로 잇는다 | ferny 09-09 파생 | **네 단계 전부 구현됨** (2026-09-09, 미출시). 명세 `DESIGN_RealityMode.md` §15. 독립 기능이 아니라 이미 있는 몰입 모드의 **두 번째 칸**으로 들어갔다. 둘이 실제로 걸어가 판이 끝날 때까지 서 있고, 그동안 오간 말이 창 아래 띠로 뜬다. 상대가 사라지면 판도 끝나고, 손님과 둔 판은 우호도를 움직인다. **인게임 확인 전** |
