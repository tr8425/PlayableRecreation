...
| 6 | **대결 상대를 식민자 중에서 고르기** — 난이도는 고른 폰의 능력치에서 뽑는다 | ferny 09-09 | **구현됨** (2026-09-09, 미출시) — 몰입 모드 2칸 1단계로 들어갔다. 상대를 고르는 FloatMenu, 머리글의 `PR.Header.OpponentNamed`, 상대 실력으로 짚어 주는 난이도까지. 상대를 붙잡는 것은 7번의 2단계다 |
| 7 | **몰입 모드 2칸** (예전 "리얼리티 모드") — 상대를 고른 뒤 두 폰을 판 앞에 붙잡아 두고, 대화·사교 다툼·상대의 죽음·방문객까지 이야기로 잇는다 | ferny 09-09 파생 | **1단계 구현됨** (2026-09-09). 명세 `DESIGN_RealityMode.md` §15. 독립 기능이 아니라 이미 있는 몰입 모드의 **두 번째 칸**으로 들어갔다. 2단계(붙잡기)부터 남음 |
