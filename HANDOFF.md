...
- 인게임 확인: **v1.1.0 QA 통과** (2026-09-08) — VFE 다트·펀칭백·룰렛·아케이드,
  Casino 슬롯, 궤도 지표 줌까지 실제로 열어 봤다. 큰 문제 없음.
  스크린샷에서 잡힌 것 하나(당기기 전 릴이 7-7-7 로 서 있던 것)는 고쳤다.
  남은 확인거리: Gloomy 가구 3종 진입, 세 모드를 모두 끈 판(로그에 빨간 줄 0)
...
1. **`git push`** — QA 를 통과했으니 이제 올려도 된다. 저장소는 비공개다
2. **v1.1.0 재업로드** — 목요일 저녁 소프트 런칭. 재업로드는 웹의 언어별 설명을
   건드리지 않으므로, 설명을 바꿨다면 웹에서 언어 탭마다 직접 붙여 넣어야 한다.
   순서: 스크린샷(`mod_collage.jpg` · `surface_zoomed.jpg`)을 캐러셀에 올리고 →
   설명문의 `{{SS6_MOD}}` 자리를 그 업로드 URL 로 바꾸고 → 언어별로 붙여 넣고 →
   변경 기록에 `Workshop/changenotes_1.1.0.txt` 를 쓴다
3. **남은 QA** — Gloomy 가구 3종 진입, 세 모드를 모두 끈 판(로그 빨간 줄 0)
