...
- **스크린샷** — `Workshop/screenshots/` 가 원본이다. 캐러셀에 올린 뒤, 설명문에 남아 있는
  `{{...}}` 자리를 그 이미지의 **직접 주소**로 바꾼다.
  모드 지원은 낱장으로 걸지 않는다 (`mod_collage.jpg` 한 장). 다섯 장을 따로 걸면
  "저 모드가 있어야 하나" 로 읽힌다

  주소 따는 법이 함정이다. 브라우저 주소창의 `.../sharedfiles/managepreviews/?id=...`
  는 **관리 페이지 주소**라 `[img]` 안에 넣으면 이미지가 아니라 깨진 링크가 된다.
  올바른 것은 항목 페이지에서 그 이미지를 **우클릭 → 이미지 주소 복사** 로 얻는

  ```
  https://images.steamusercontent.com/ugc/<숫자>/<대문자 해시>/
  ```

  꼴이다 (끝의 `/` 까지). 페이지에서 눈으로 찾기 번거로우면 항목 페이지를 받아
  `images.steamusercontent.com/ugc/...` 를 전부 뽑은 뒤, 각 이미지의 크기나 지문으로
  로컬 파일과 짝지어도 된다 — 2026-09-08 에 그렇게 찾았다
...
- [ ] **지원 모드를 전부 끈 판**에서도 켜 봤다 (로그에 빨간 줄이 없어야 한다)
